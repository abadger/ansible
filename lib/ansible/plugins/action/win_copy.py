...
class ActionModule(ActionBase):
    ...
    def _copy_single_file(self, local_file, dest, source_rel, task_vars, tmp):
        ...
        copy_result = self._execute_module(module_name="copy", module_args=copy_args, task_vars=task_vars, tmp=tmp)
    ...
    def _copy_zip_file(self, dest, files, directories, task_vars, tmp):
        ...
        module_return = self._execute_module(module_args=copy_args, task_vars=task_vars, tmp=tmp)
        return module_return

    def run(self, tmp=None, task_vars=None):
        ''' handler for file transfer operations '''
        # TODO: merge common code with copy
        ...
        query_return = self._execute_module(module_args=query_args, task_vars=task_vars, tmp=tmp)
        ...
        if len(query_return['files']) == 1 and len(query_return['directories']) == 0:
            ...
            copy_result = self._copy_single_file(file_src, dest, file_dest, task_vars, tmp)
            ...
            result.update(self._copy_zip_file(dest, source_files['files'], source_files['directories'], task_vars, tmp))
            result['changed'] = True
        else:
